...
| `fb_index_advice` | `db`, `query`, optional `recheck_of` (advisory id) | Plan analysis (C.2): finds **natural (full) table scans** with sargable predicates and proposes `CREATE INDEX` DDL with an estimate-only benefit (Firebird cannot simulate an uncreated index; assumed distinctness, labeled as such). Conservative: subqueries, `OR`, ambiguous columns and function-wrapped predicates yield "no advice" with a reason. **DDL is never applied here** — run it via `fb_write`. Existing indexes with the same leading columns suppress the advice. `recheck_of` re-plans a previously advised query and reports which scans were resolved |
...
| `fb_trends` | `db`, optional `hours` (default 24), `threshold_gb` | C.4 capacity trends from the sampler history (one sample per database every `trends.interval_seconds`, default 300 s, stored under `<state.dir>/trends/`): size least-squares projection ("threshold in ~N days"), attachment-spike anomaly flag (mean+3σ), window IO deltas (engine restarts detected via counter resets). History starts when the kernel first ran with the sampler enabled (`trends.disabled: true` opts out; `trends.retention_days` default 30). **Side effect, stated plainly:** each tick briefly attaches to every configured database; even read-only-transaction attachments make the engine touch the file (housekeeping on attach/detach — normal client behavior, but byte-level file comparisons will see changes). Disable if a database must stay byte-idle |
| `fb_diff_schema` | `db`, optional `vs_db`, `use_snapshot`, `save_snapshot` | C.3 schema diff: two registered databases, or db vs its stored snapshot (snapshot-vs-now is the default with no `vs_db`). Grouped "would need CREATE / ALTER / DROP" output; `save_snapshot` persists the current schema under `<state.dir>/schema-snapshots/`. Read-only |
| `fb_diff_data` | `db`, `vs_db`, `table`, optional `row_cap` (default 100k), `sample_rows` (default 10) | C.3 bounded data diff: streams both databases ordered by the table's primary key, counts only-in-A / only-in-B / differing rows with samples. **Refuses** tables above `row_cap` instead of truncating. Requires a PK on both sides. Read-only |
